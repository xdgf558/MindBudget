...
- Active phase: **G1 is In Progress under DEC-COM-095 through DEC-COM-102 with
  ...
  on 2026-08-29. Independent review approved head `4ea7cd9` and raised one P2 cross-segment
...
- [x] **C2-03 — Purchase and restore flows.** The merged implementation has one
  `EntitlementStore` authority, verified status/renewal mapping,
  ...
  file above 85%; the strict local wall-clock signal separately passed 10/10. PR #30 passed
  independent review and green CI, then merged to `main` as `3fc72b4` on 2026-08-13. No current
...
- [x] **C4B-02 — Sync implementation.** Reviewed head `0024507` passed GitHub Actions run
  `32490174014`; PR #59 merged as `211dff2`. The merged boundary includes
...
- [x] **C4C-02 — Image acquisition and lifecycle.** Done after reviewed head `43c3a35`, green
...
- [x] **C4C-03 — OCR and pre-model privacy.** Done after reviewed head `92ed3a7`, green GitHub
...
- [x] Fixed event/property allow-list, rotating pseudonymous ID, delete secret, opt-out/reset/delete,
  ...
  grouped complete-delete association, made repeated Disable storage-free, uses the user calendar,
  separates local commit failure from transport backoff, and made the dormancy gate self-testing/
  fail-closed. Exact-source full validation passes Release, the strict 10,000-row Dashboard
...
- [x] Independent serverless ingest, unknown/free-text rejection, 90-day TTL, deletion API,
  ...
  bounded cleanup. Exact remediation head `72abf4b` passed independent review and hosted run
  `33176551566`; PR #78 merged it as `4715054` without entering C5-03.
...
- [x] Define App Store metric workflow, survey, exact numerator/denominator/confidence intervals,
  ...
  Independent review approved head `4ea7cd9` and raised the P2/P3 findings. Remediation head
  `0c61427` applied both, passed hosted run `33211270363`, and PR #80 merged it as `a587f42`
  without a pre-merge rereview. PR #81's post-merge closeout review confirmed that exact delta
  without enabling capture, creating an evidence result, or deciding G1.
...
- [x] Independently review and merge the account/Eval evidence. Review read all 24 outputs and
