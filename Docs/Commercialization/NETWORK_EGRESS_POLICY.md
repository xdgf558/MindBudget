...
| StoreKit lifecycle | Partially accepted in C2-02; purchase/restore/status pending | Configuration/Sandbox/TestFlight/Production isolated | Apple-managed StoreKit API; no app-owned domain or raw receipt endpoint | Accepted now: `Product.products(for:)`, `AppTransaction.shared`, `Storefront.current`, `Transaction.currentEntitlements`, `Transaction.updates`; purchase/restore/finish/status APIs remain blocked | Accepted Product IDs plus Apple-required product/transaction environment metadata only; no ledger/note/receipt content | Typed `Product`, verified current transaction, raw revocation/expiration facts, environment and storefront | No purchase/restore control yet; system account state may update current entitlements | Apple platform policy; deletable environment/storefront presentation cache only, never authority | Product load failure uses exact-context presentation cache without erasing verified entitlement; revoked/mixed/unverified/unknown authority input grants no rights; C2-03 maps expiration with subscription status | REQ-STOREKIT-STATE-001, REQ-STOREKIT-LIFECYCLE-001 / COM-C2; DEC-COM-016 |
