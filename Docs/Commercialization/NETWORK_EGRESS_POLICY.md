...
| StoreKit lifecycle | Accepted through the review-pending C2-03 implementation; no customer UI or distribution authorization | Configuration/Sandbox/TestFlight/Production isolated | Apple-managed StoreKit API; no app-owned domain or raw receipt endpoint | `Product.products(for:)`, `AppTransaction.shared`, `Storefront.current`, `Transaction.currentEntitlements`, `Transaction.updates`, `Product.SubscriptionInfo.Status.updates`, `Product.SubscriptionInfo.status(for:)`, `Transaction.subscriptionStatus`, `Transaction.unfinished`, `AppStore.canMakePayments`, explicit `Product.purchase()`, explicit `AppStore.sync()`, and `Transaction.finish()` only after authoritative publish | Accepted Product IDs plus Apple-required product, transaction, renewal, ownership, environment, and storefront metadata only; no ledger/note/receipt content | Typed `Product`; verified status transaction and renewal information; verified current/update/unfinished/purchase transaction facts; transaction and subscription-status signals; pending/cancel/error; raw revocation/expiration; environment/storefront | One lifecycle task supervises both update sequences; each status signal triggers a fresh full reconciliation through the same authority and cannot grant directly. Purchase and restore exist only as explicit typed programmatic seams; no current view invokes them and no implicit restore prompt exists | Apple platform policy; deletable environment/storefront presentation cache only, never authority; failed finish remains unfinished for later StoreKit lifecycle retry | Product/status load failure, unknown/mixed/unverified input, incomplete free authority, and finish failure fail closed; subscribed/verified grace grant Pro while retry/expired/revoked/pending do not; customer UI, C2-04 environment proof, and distribution remain blocked | REQ-STOREKIT-STATE-001, REQ-STOREKIT-LIFECYCLE-001 / COM-C2; DEC-COM-016/017 |
...
transport accepted incrementally by C2-01/C2-02 and the review-pending C2-03 implementation; it
