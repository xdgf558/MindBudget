...
| First-party telemetry ingest | C5-01 dormant local client Done through PR #76 (`68304ad`); domain UNVERIFIED; no production construction/capture/transport and zero current egress | Dev/staging/prod typed but no environment is connected | Exact first-party domain TBD; no wildcard; `UnavailableTelemetryTransport` only | No accepted endpoint or method; batched upload and deletion paths remain C5-02 | Closed enum event/action/outcome plus schema/environment/validated app version, rotating UUID pseudonym, deletion handle, occurrence time; ordinary upload envelopes contain one non-reused pseudonym; the complete-delete request deliberately groups retained per-generation UUID/32-byte proofs and C5-02 may use that association only transiently for deletion; no caller dictionary, free text, ledger, merchant, amount, note, receipt image/OCR/model evidence, StoreKit ID, or CloudKit envelope | Typed acceptance/rejection/retry, `.persistenceFailed`, and deletion success only; corrupt local cleanup has a separate no-remote-claim result; no live response in C5-01 | Missing-state default-off and repeated Disable create no file/key/write; no setting/call site yet. Opt-out clears unsent events, retires the pseudonym, and re-enable cannot reuse it in an upload envelope; four retained generations can block re-enable until expiry/Delete and C5-04 owns truthful guidance | Local proof target 90 user-calendar days; real server TTL/delete and in-flight opt-out cancellation remain unverified C5-02/C5-04 work. Reset rotates; confirmed remote delete must precede readable proof destruction; corrupt local file/key deletion remains available without a remote claim; C5-02 upload acceptance and proof deletion must be idempotent after local commit/cleanup failure | Encrypted 256-event queue, 20-event single-generation batch, bounded transport backoff, distinct local persistence failure, sticky collection/overwrite corruption with unconditional local deletion; any failure leaves the product fully usable and cannot affect entitlement/budget | REQ-R1-TELEMETRY-001 / C5-01; DEC-COM-056/057/058/059 |
...
default-off typed client plus encrypted local persistence and deterministic tests. The app target
contains no `TelemetryClient` construction or capture call, and `UnavailableTelemetryTransport`
contains no URL or network primitive. The telemetry row remains domain/endpoint UNVERIFIED and the
accepted app-owned HTTP(S) set is unchanged. Review remediation makes corrupt local file/key
...
log, or reuse. C5-02 must establish the exact receiver and in-flight opt-out contracts before any
adapter or egress can be reviewed; that receiver must deduplicate accepted event IDs and identical
proof-deletion retries when remote success precedes local acknowledgement or cleanup.
Exact final head `d937dc8` passed GitHub Actions run `33085630481`, and PR #76 merged C5-01 as
`68304ad`. This closeout accepts no telemetry host, path, method, adapter, receiver, production
construction, capture call, customer control, or App Privacy change. C5-02 awaits a separate
explicit owner entry; the accepted app-owned HTTP(S) set and current telemetry egress remain zero.
