...
| First-party telemetry ingest | C5-01 Done through PR #76 (`68304ad`); C5-02 implementation pending review/CI/merge; adapter dormant, default-off, unconstructed, and zero capture/customer egress | Exact Development/Staging/Production compile-time contexts; only Development Worker/D1 deployed and probed; Staging D1 isolated but unmigrated/undeployed; Production D1 unprovisioned and Worker undeployed | `mindbudget-telemetry-dev.yehao1105.workers.dev`; `mindbudget-telemetry-staging.yehao1105.workers.dev`; `mindbudget-telemetry.yehao1105.workers.dev`; no wildcard/caller URL/redirect; `UnavailableTelemetryTransport` remains the app default | Anonymous `POST /v1/events` (max 20 events/32 KiB) and proof-authenticated `POST /v1/delete` (max 4 proofs/2 KiB); exact host/path, empty status-coded responses, no auth/cookie/query/content encoding | Closed enum event/action/outcome plus schema/environment/validated app version, rotating UUID pseudonym, deletion handle, occurrence time; ordinary uploads contain one generation; complete delete groups retained UUID/32-byte proofs only in request-local memory/SQL parameters; no arbitrary map/free text/ledger/merchant/amount/note/receipt/OCR/model evidence/StoreKit ID/CloudKit envelope | Upload 202/409/429/5xx mapped to typed acceptance/rejection/retry; delete 204 only after all proofs validate; identical retries idempotent; same event UUID with changed facts atomically conflicts; late matching upload after deletion is accepted-but-discarded; response body bounded to 1 KiB and must be empty | Missing/repeated Disable remains zero-write; no customer setting/capture call. Opt-out clears unsent events, retires the identity, then best-effort cancels active upload; it does not claim to recall an edge-accepted request. C5-04 owns bilingual controls/disclosure and retained-generation guidance | Independent tombstones prevent late resurrection. Events/identities/tombstones expire no later than server acceptance + 90 x 24 UTC hours through hourly bounded indexed cleanup; delete association is never persisted/logged/reused; confirmed delete precedes local proof destruction. Development live probe ended at 0 events/0 identities/2 tombstones | Exact JSON keys plus duplicate-key/UTF-8/depth/node/body/batch bounds; D1 uniqueness/transaction authority; edge IP/pseudonym limiters only abuse buffers; invocation logs off; sampled custom logs contain closed component/environment/route/reason only; any failure leaves entitlement/budget/local use unaffected | REQ-R1-TELEMETRY-001 / C5-01/C5-02; DEC-COM-056/057/058/059/060 |
...
tombstones, a 90 x 24-hour maximum server TTL, closed monitoring, and best-effort in-flight upload
cancellation. Only Development version `1c162a57-8789-4f7f-9fec-f2c484e9f4f2` is deployed and
...
Current customer telemetry egress remains zero; C5-03/C5-04 remain blocked.
