...
| First-party telemetry ingest | C5-01 local client candidate only; domain UNVERIFIED; no production construction/capture/transport and zero current egress | Dev/staging/prod typed but no environment is connected | Exact first-party domain TBD; no wildcard; `UnavailableTelemetryTransport` only | No accepted endpoint or method; batched upload and deletion paths remain C5-02 | Closed enum event/action/outcome plus schema/environment/validated app version, rotating UUID pseudonym, deletion handle, occurrence time; deletion request contains only per-generation UUID and 32-byte proof; no caller dictionary, free text, ledger, merchant, amount, note, receipt image/OCR/model evidence, StoreKit ID, or CloudKit envelope | Typed acceptance/rejection/retry and deletion success only; no live response in C5-01 | Missing-state default-off; no setting/call site yet. Opt-out clears unsent events, retires the pseudonym, and re-enable cannot reuse it; disclosure/UI remain C5-04 | Local proof target 90 days; real server TTL/delete remain unverified C5-02/C5-04 work. Reset rotates; confirmed remote delete must precede local proof destruction | Encrypted 256-event queue, 20-event single-generation batch, bounded backoff, sticky corruption; any failure leaves the product fully usable and cannot affect entitlement/budget | REQ-R1-TELEMETRY-001 / C5-01; DEC-COM-056 |
...
The owner explicitly entered COM-C5 on 2026-08-27. DEC-COM-056 and C5-01 add only a dormant,
...
accepted app-owned HTTP(S) set is unchanged. C5-02 must establish the exact receiver contract before
any adapter or egress can be reviewed.
