...
| Free iCloud sync | C4B-01 architecture Accepted and Done through PR #57 (`90a1e66`); no runtime/container/entitlement/import/request/deployment | Exact future `iCloud.com.xdgf558.MindBudget` identifier accepted but not created/provisioned; entitlement-selected Development/Production environments and same-named private custom zone remain separate | Apple-managed CloudKit via `CKSyncEngine`; no app-owned HTTP(S), public/shared database, or managed SwiftData mirroring | Future typed versioned records in one private custom zone; C4B-01 and prerequisite maintenance create none | Allow-listed authoritative envelopes only; typed content in `CKRecord.encryptedValues`; never receipt image/OCR/local intermediate/recovery artifact/log/StoreKit/config-cache data | Typed envelope, logical tombstone, opaque engine state, closed reason code; no raw ledger logging | Free default-off explicit opt-in using the accepted bilingual financial-record/notes/reflections disclosure; account switch pauses and needs re-consent; engine is not initialized before consent | Disable retains local data; cloud-wide deletion is separately confirmed and tombstone-backed; C4B-03 must prove retention/compaction | Local use stays usable for offline/account/quota/CloudKit/conflict/key-reset failure; no automatic destructive resolution or key-reset purge/reupload | REQ-ICLOUD-001 / C4B-01 design; DEC-COM-006/028 accepted |
