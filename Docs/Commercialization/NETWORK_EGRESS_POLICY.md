...
(`1ebb36c`) and intentionally contains no adapter or URL, so it did not move the current empty
Release HTTP(S) row. C3-03B is In Progress but may request the exact adapter exception only in the
same reviewed change that proves its first-party Worker/key, privacy, traffic, and binary evidence.
