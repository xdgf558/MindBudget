...
| First-party telemetry ingest | C5-01 Done through PR #76 (`68304ad`); C5-02 Done through reviewed head `72abf4b`, green run `33176551566`, and PR #78 merge `4715054`; adapter dormant, default-off, unconstructed, and zero capture/customer egress | Exact Development/Staging/Production compile-time contexts; only Development Worker/D1 deployed and probed; Staging D1 isolated but unmigrated/undeployed; Production D1 unprovisioned and Worker undeployed | `mindbudget-telemetry-dev.yehao1105.workers.dev`; `mindbudget-telemetry-staging.yehao1105.workers.dev`; `mindbudget-telemetry.yehao1105.workers.dev`; no wildcard/caller URL/redirect; `UnavailableTelemetryTransport` remains the app default | Anonymous `POST /v1/events` (max 20 events/32 KiB) and proof-authenticated `POST /v1/delete` (max 4 proofs/2 KiB); exact host/path, empty status-coded responses, fixed `User-Agent: MindBudget`, absent/empty `Accept-Language`, no auth/cookie/query/content encoding | Closed enum event/action/outcome plus schema/environment/validated app version, rotating UUID pseudonym, deletion handle, occurrence time; ordinary uploads contain one generation; complete delete groups retained UUID/32-byte proofs only in request-local memory/SQL parameters; no build/OS/locale metadata, arbitrary map/free text/ledger/merchant/amount/note/receipt/OCR/model evidence/StoreKit ID/CloudKit envelope | Upload 202/409/429/5xx mapped to typed acceptance/rejection/retry; delete 204 only after all proofs validate; identical retries idempotent; same event UUID with changed facts atomically conflicts; late matching upload after deletion is accepted-but-discarded; response body bounded to 1 KiB and must be empty; C5-04 must make fixed 404/405/421 policy failures terminal before construction | Missing/repeated Disable remains zero-write; no customer setting/capture call. Opt-out clears unsent events, retires the identity, then best-effort cancels active upload; it does not claim to recall an edge-accepted request. C5-04 owns bilingual controls/disclosure and retained-generation guidance | Independent tombstones prevent late resurrection. Events/identities expire by acceptance + 90 x 24 UTC hours; tombstones use an earlier-or-equal UTC-day bucket shared across requests, retaining broad expiry day but no request-unique timestamp/group. Hourly cleanup repeats bounded 1,000-row transactions until drained; confirmed delete precedes local proof destruction. The Development live probe ended at 0 events/0 identities/2 tombstones on pre-remediation Worker `1c162a57-8789-4f7f-9fec-f2c484e9f4f2`; it predates DEC-COM-061 and is not current-source probe evidence | Exact JSON keys plus strict JSON-only whitespace, duplicate-key/UTF-8/depth/node/body/batch bounds; D1 uniqueness/transaction authority; fixed metadata headers; edge IP/pseudonym limiters only abuse buffers; invocation logs off; sampled custom logs contain closed component/environment/route/reason only; any failure leaves entitlement/budget/local use unaffected | REQ-R1-TELEMETRY-001 / C5-01/C5-02; DEC-COM-056/057/058/059/060/061/062 |
...
C5-03 adds no network row or egress path. Independent review approved exact dormant-evidence head
`0c61427`, GitHub Actions run `33211270363` passed, and PR #80 merged it as `a587f42` under
DEC-COM-065. The read-only aggregate and offline builder remain unreachable from the live Worker;
no customer capture, real evidence bundle, G1 decision, App Privacy change, deployment, or release
is inferred. C5-04 awaits explicit owner entry before any telemetry activation or operations work.
...
first-party hosts, but the app target still has no `TelemetryClient` construction or capture call,
and `UnavailableTelemetryTransport` remains the default. Review remediation makes corrupt local file/key
...
`68304ad`. The C5-02 source increment still accepts no production client construction, capture call,
...
zero. The owner entered C5-03 on 2026-08-29; reviewed head `0c61427` passed run `33211270363`, and
PR #80 merged it as `a587f42`. Its evidence builder is offline and its D1 receipt funnel is an
unexposed read-only aggregate: it adds no host, path, request, client construction, or capture.
