...
Current REQ-G1-001 still requires the fixed bilingual three-way comparative Eval blind score; the
reviewed capture delivery alone does not satisfy it.
...
| REQ-CLOUD-AUTH-001 | Deferred by DEC-COM-104; absent from local-only launch | v1.4 §§15.4–15.6, COM-C7/C10 | Future fresh G1 plus COM-C7/C10 | Historical App Attest/JWS/current-authority design remains preserved, but no backend session, consumable fact, or cloud request is implemented or enabled. Re-entry requires fresh owner approval and `PROCEED_TO_R2` | SPEC-007/013/014 accepted; DEC-COM-095/096/104 | Deferred cloud path only |
| REQ-CLOUD-CONSENT-001 | Deferred by DEC-COM-104; production not admitted | v1.4 §0.3, §12, §16 | Future fresh G1 plus COM-C8/C12 | Historical consent design names OpenAI `gpt-5.6-luna`, exact data/purpose, Global processing, no voluntary training, and standard up-to-30-day abuse-monitoring retention. DEC-COM-097 admitted only synthetic Eval; DEC-COM-104 keeps every production provider route absent from the local-only candidate | SPEC-012 accepted; DEC-COM-095/096/097/104 | Deferred cloud path only |
| REQ-CLOUD-USAGE-001 | Deferred by DEC-COM-104; no active credit offer | v1.4 §0.2–0.3, §14 | Future fresh G1 plus COM-C9/C11/C12 | DEC-COM-095/096 preserve the historical 10-starter and 10/25/65-use-card design, one-year lots, displayed-valid-result accounting, refund isolation, and 50% margin goal. DEC-COM-104 makes those terms non-current: no Luna credit is granted or sold, no Product ID or ledger is created, and any future proposal must supply fresh value/economics evidence plus a bootstrap-safe breaker rather than reuse the below-1,000 rule unchanged | SPEC-012/014 accepted; DEC-COM-092/093/094/095/096/104 | Deferred cloud path only |
| REQ-G1-001 | Accepted; G1 Done under DEC-COM-105 at `DEFER_LUNA_CREDITS_KEEP_LOCAL_PRO` | v1.4 §19.2–19.5 plus DEC-COM-092 through DEC-COM-105 | Completed G1 decision; Watch remains separately gated | DEC-COM-094 preserves reviewed PR #98 (`9226985`, run `33570570896`, merge `6e2d242`) and historical `INSUFFICIENT_QUOTE_EVIDENCE`. DEC-COM-095/096 record the US$4.99 local-Pro/trial policy and historical Luna/card offer. DEC-COM-097/098/099 preserve synthetic-only account admission and the independently reviewed 24/24 Luna run. DEC-COM-100/101 preserve the physical three-way harness/capture delivery. DEC-COM-102 records completed-review SHA-256 `d2b9310f4471400825e666009f646a190d8ac2819f859c8e38d58ec05cbf040e` and deterministic `NON_PASS`: zero materially preferred Luna cases, no qualifying bilingual task, and no second eligible blind score. DEC-COM-103 closes that result-recording delivery. DEC-COM-104 supplies the owner deferral. PR #106 remediation head `961acc0` passed independent rereview and hosted run `33724552517`, and merge `fdd511b` retained it as second parent. DEC-COM-105 closes G1 without `PROCEED_TO_R2`: Luna/cards and COM-C7 through COM-C11 remain deferred, production remains false, and separately owner-entered local-only COM-C12 is eligible but unentered | SPEC-013/014 accepted; DEC-COM-092/093/094/095/096/097/098/099/100/101/102/103/104/105; `G1_UNIT_ECONOMICS_PACKET.md`; `G1_LUNA_EVAL.md`; `G1_OPENAI_ACCOUNT_EVIDENCE.md`; `G1_LUNA_EVAL_RESULT_2026-09-02.json`; `G1_THREE_WAY_EVAL.md`; `G1_APPLE_ON_DEVICE_EVAL_TRANSCRIPT_2026-09-02.jsonl`; `G1_THREE_WAY_BLIND_REVIEW_2026-09-02.json`; `G1_THREE_WAY_REVIEW_SIDECAR_2026-09-02.json`; `G1_THREE_WAY_REVIEW_RESULT_2026-09-02.json`; `Scripts/g1_unit_economics.py`; `Scripts/g1_luna_eval.py` | P0 / completed G1 decision; future local-only COM-C12 entry |
...
## G1 owner disposition — 2026-09-03

DEC-COM-104 supplies the later owner disposition `DEFER_LUNA_CREDITS_KEEP_LOCAL_PRO`. PR #106
remediation head `961acc0` passed independent rereview and hosted run `33724552517`, then merged as
`fdd511b` with that head as second parent. DEC-COM-105 closes the reviewed record and marks
REQ-G1-001 accepted and G1 Done at the deferral outcome. The frozen `NON_PASS` and
production-false evidence remain unchanged. REQ-CLOUD-AUTH-001, REQ-CLOUD-CONSENT-001, and
REQ-CLOUD-USAGE-001 are deferred rather than completed, and COM-C7 through COM-C11 are not entered.
A separately owner-entered local-only COM-C12 review is eligible but remains unentered.
The preserved historical chain remains DEC-COM-095, DEC-COM-096, DEC-COM-097, DEC-COM-098,
DEC-COM-099, DEC-COM-100, DEC-COM-101, DEC-COM-102, and DEC-COM-103; DEC-COM-104 changes only the
current owner disposition and downstream authority. The PR #102 capture delivery anchors remain
`bb939d0`, run `33628847476`, merge `2254902`, and original blind surface
`bcbf943ba7d6a1a9d18442efc38e760cc798c30e8674c8d877f9e0cb751ab2a5`.
Historical scope still records the sole `gpt-5.6-luna`, US$4.99 and consumable options, 50% margin,
typical/P50 and peak/P95 economics, and the three-way comparative Eval without activating them.
It also preserves, only as historical policy evidence, zero Luna credits in the trial,
one user-calendar year of credit validity, charging only for an analysis ultimately displayed
after a user action, no Luna access for an ordinary test user, and a separate Apple App Review allowance.
DEC-COM-104 defers those cloud-credit mechanics with the offer.
The reviewed Luna delivery remains head `323d8d7`, hosted run `33593253561`, merge `7a473d2`, and
the former `EVAL_REVIEWED_PENDING_STOREFRONT_EVIDENCE` checkpoint.

## G1 owner-disposition closeout — 2026-09-03

DEC-COM-105 closes only the reviewed DEC-COM-104 disposition delivery. It preserves the
comparative `NON_PASS`, production-false boundary, lack of `PROCEED_TO_R2`, and deferred status of
all cloud requirements and COM-C7 through COM-C11. Optional iCloud and first-party telemetry are
part of the future local-only C12 matrix only when enabled in the final candidate; the closeout
does not authorize either channel. Local-only COM-C12 still requires a separate owner entry.
