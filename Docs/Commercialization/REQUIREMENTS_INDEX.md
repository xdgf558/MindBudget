...
|---|---|---|---|---|---|---|
| REQ-R1-NET-001 | Active; C3-03A implementation complete pending review, no transport yet | v1.4 §1 rules 21–23, §4.7, §15.1 | COM-C0B policy; COM-C3 config; COM-C5 telemetry; COM-C6 verification | DEC-COM-021 accepts exact dev/staging/prod hosts, anonymous `GET /v1/config`, app/config-version-only outbound metadata, Ed25519 exact-byte verification, seven-day maximum validity, rollback/equivocation rejection, bounded signed cache, and conservative fallback. C3-03A implements only the verifier/cache core and retains an empty Release HTTP(S) allow-list; C3-03B must prove the fixed adapter, key/Worker provenance, privacy/log/TTL/redirect controls, captured traffic, and final binary before any egress can be accepted. No ledger/content request in R1 | SPEC-001/012 accepted; DEC-COM-021 | P0 / COM-C3, COM-C6 and 1.0 |
