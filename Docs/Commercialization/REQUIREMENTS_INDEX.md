...
| REQ-ICLOUD-001 | Active; C4B-01 proposed architecture candidate pending owner/independent review, not implemented | v1.4 §9.7, §16.5.2, COM-C4B | C4B-01 design; C4B-02/03; verify C6/C12 | `ICLOUD_SYNC_CONTRACT.md` defines default-off Free custom records in one private custom zone through `CKSyncEngine`, stable UUID-derived names, typed/versioned envelopes, logical tombstones, per-record lineage/parent-digest plus server-owned change-tag and encrypted semantic-digest replay detection, divergent-conflict quarantine, separate engine-state/outbox persistence, account pause/re-consent, and local-first offline/quota failure. It explicitly excludes managed SwiftData mirroring and requires `cloudKitDatabase: .none` before entitlement/import. C4B-02/03 still owe container, multi-device, Dashboard, deletion, account, quota, and release evidence | SPEC-003/012 accepted; SPEC-018 resolved; DEC-COM-006 accepted; DEC-COM-028 Proposed | P0 / COM-C4B and 1.0 |
