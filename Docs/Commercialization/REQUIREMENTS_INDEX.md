...
| REQ-RECEIPT-PIPELINE-001 | Active; C4C-01 seams/evidence implemented pending review, pipeline not implemented | v1.4 §9.6.1–9.6.9 | COM-C4C | C4C-01 centralizes the future receipt rights, defines unavailable/deterministic/local-model-with-deterministic-fallback tiers, and adds integer rule sample/confidence evidence without image access. Five acquisition/OCR/extraction/validation/confirmation stages; geometry/order/confidence; exact validation; duplicate warning; cancellation/resource limits; 60+ fixed receipts; 20-image stability; no save before confirmation remain C4C-02 through C4C-05 | SPEC-010 resolved; SPEC-015 accepted; DEC-COM-044; `COM_C4C_EXECUTION_PACKET.md` | P0 / local Pro and 1.0 |
| REQ-RECEIPT-PRIVACY-001 | Active, not implemented; C4C-01 adds no image/content path | v1.4 §1 rules 26–27, §9.6.6 | COM-C4C; verify C6/C8/C12 | Zero raw image/OCR/card/line-name transfer to cloud; sensitive-pattern removal before model; local temp cleanup; no receipt image in iCloud/logs; confirmation before persistence. C4C-01 retains `enableReceiptImport = false` and creates only pure access/tier/evidence values | SPEC-010/012 accepted; DEC-COM-044; `COM_C4C_EXECUTION_PACKET.md` | P0 / local Pro, cloud AI, 1.0 |
