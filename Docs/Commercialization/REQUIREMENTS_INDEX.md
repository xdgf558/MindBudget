...
| REQ-ICLOUD-001 | Active; C4B-01 architecture Accepted and Done through PR #57 (`90a1e66`), not implemented | v1.4 §9.7, §16.5.2, COM-C4B | C4B-01 design; C4B-02P prerequisites; C4B-02/03; verify C6/C12 | `ICLOUD_SYNC_CONTRACT.md` defines default-off Free custom records in one private custom zone through `CKSyncEngine`, stable UUID-derived names plus a closed recurring-occurrence grammar, revision-1/no-parent genesis, typed/versioned envelopes, logical tombstones, exact parent/change-tag/semantic-digest replay detection, divergent-conflict quarantine without an automatic winner, separate engine-state/outbox persistence, account pause/re-consent, and local-first offline/quota failure. It freezes the exact future container/disclosure input, excludes managed SwiftData mirroring, and requires repository-wide `cloudKitDatabase: .none` plus centralized `ModelContainer` construction before entitlement/import. C4B-02/03 still owe implementation, container/provisioning, multi-device, Dashboard, deletion, account, quota, and release evidence | SPEC-003/012 accepted; SPEC-018 resolved; DEC-COM-006/028 accepted | P0 / COM-C4B and 1.0 |
