...
| REQ-ICLOUD-001 | Active; C4B-01, C4B-02P, and C4B-02 Done; C4B-03 product capability merged through PR #61 `0f749ce`; PR #62 calibration merged as `0128682`; evidence In Progress with the physical same-account gate waived | v1.4 §9.7, §16.5.2, COM-C4B | C4B-01 design; C4B-02P prerequisites; C4B-02/03; verify C6/C12 | `ICLOUD_SYNC_CONTRACT.md` defines default-off Free custom records in one private custom zone through `CKSyncEngine`, stable UUID-derived names plus a closed recurring-occurrence grammar, revision-1/no-parent genesis, typed/versioned envelopes, logical tombstones, exact parent/change-tag/semantic-digest replay detection, divergent-conflict quarantine without an automatic winner, separate engine-state/outbox persistence, account pause/re-consent, and local-first offline/quota failure. PR #58 merged repository-wide `cloudKitDatabase: .none` and centralized-container prerequisites as `6f5fded`. Reviewed C4B-02 head `0024507` passed GitHub Actions run `32490174014`; PR #59 merged the Schema V6 runtime as `211dff2`. PR #60 merged the reviewed entry closeout as `7138a9c`. Reviewed C4B-03 product head `f49de94` passed run `32571676058`, and PR #61 merged it as `0f749ce`. The merged capability includes exact Development/Production entitlements, explicit no-content conflict resolution, durable whole-zone deletion with local-fact preservation, retained-copy reimport confirmation, explicit sticky recovery, and exact remote-notification background configuration. Deterministic local tests, corrected migration/free-tier regression, signed builds, and complete local validation are joined by a 33/33 physical Development suite whose real case creates the custom zone, sends/fetches an encrypted private record, requires confirmed reimport, deletes the zone, and preserves the local expense. Read-only Dashboard evidence confirms one unindexed encrypted app field in Development and no app record type/deployed schema in Production. The prepared signed two-device harness did not converge because the devices use different iCloud Apple Accounts. The stopped attempt remains a non-pass; DEC-COM-039 permanently waives only a physical same-account rerun as exit evidence while deterministic conflict/no-winner behavior remains required. Physical account/quota/offline/push evidence, distribution signing, Production deployment, and release proof remain open | SPEC-003/012 accepted; SPEC-018 resolved; DEC-COM-006/028/029/031/032/033/034/035/038/039 accepted | P0 / COM-C4B and 1.0 |
