...
| REQ-MONEY-MIGRATION-001 | Active; C4A-02 Done through PR #53 `c905415`; C4A-03 blocked pending owner instruction | v1.4 §9.6.4, COM-C4A | COM-C4A | Inventory every amount; old-store backup/restore; idempotent restart; no `Double`; anomalies stop and never become zero; USD/JPY/KWD/bounds/negative fixtures. C4A-02 surrounds non-fast-path opening with a checksum manifest and closed journal; no-store means no backup; only a supported committed current-target marker with no active journal is trusted. Post-open integrity validation commits only after success, while a failure restores a verified snapshot. C4A-03 owns the exhaustive recovery/currency matrix and separate performance evidence | Current V1–V4 are already minor-unit; no destructive rewrite justified; DEC-COM-026; `COM_C4A_EXECUTION_PACKET.md` | P0 / COM-C4 and 1.0 |
