...
| REQ-ICLOUD-001 | Active; C4B-01 and C4B-02P Done; C4B-02 implementation complete pending independent review | v1.4 §9.7, §16.5.2, COM-C4B | C4B-01 design; C4B-02P prerequisites; C4B-02/03; verify C6/C12 | `ICLOUD_SYNC_CONTRACT.md` defines default-off Free custom records in one private custom zone through `CKSyncEngine`, stable UUID-derived names plus a closed recurring-occurrence grammar, revision-1/no-parent genesis, typed/versioned envelopes, logical tombstones, exact parent/change-tag/semantic-digest replay detection, divergent-conflict quarantine without an automatic winner, separate engine-state/outbox persistence, account pause/re-consent, and local-first offline/quota failure. PR #58 merged repository-wide `cloudKitDatabase: .none` and centralized-container prerequisites as `6f5fded`. C4B-02 implements Schema V6 local sync metadata, transactional fact/outbox staging, inbox-first topological application of all 12 allow-listed facts, localized opt-in/status/retry controls, private-database transport, logical tombstones, and closed account/key-reset/conflict failure. DEC-COM-031 makes account/key-reset/zone-loss pauses sticky, maps destructive `zoneNotFound` errors before retry, quarantines invalid allocation and divergent recurring claims, requires parent-owned envelope keys, and fixes Delete All as local-only until C4B-03. C4B-03 still owes entitlement/container provisioning, Dashboard and environment deployment, physical multi-device/account/quota evidence, conflict resolution UI, cloud-wide deletion, compaction/retention, and release proof | SPEC-003/012 accepted; SPEC-018 resolved; DEC-COM-006/028/029/031 accepted | P0 / COM-C4B and 1.0 |
