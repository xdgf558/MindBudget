...
| REQ-MONEY-001 | Active; C4A-01 audit complete pending review | v1.4 §1 rule 25, §9.2, §9.6.4 | Existing product; COM-C4A delta; shared with Watch | Every stored/calculated amount is `Int64` minor units plus ISO code; currency exponent matrix; overflow/cross-currency failure; money-path float gate. C4A-01 inventories all 15 `ModelCounts` tables, including explicit no-persisted-money rows, and proves that V1–V4 authoritative amounts already satisfy the minor-unit representation; the rebuildable merchant total's implicit currency ownership is the only representation cleanup assigned to C4A-02 | SPEC-015 accepted tooling boundary; `COM_C4A_EXECUTION_PACKET.md` | P0 / all financial phases |
| REQ-MONEY-MIGRATION-001 | Active; C4A-01 plan complete pending review; implementation blocked | v1.4 §9.6.4, COM-C4A | COM-C4A | Inventory every amount; old-store backup/restore; idempotent restart; no `Double`; anomalies stop and never become zero; USD/JPY/KWD/bounds/negative fixtures. C4A-01 rejects a destructive rewrite and assigns one pre-open sidecar/journal envelope: no-store means no backup; only a supported, parseable, committed app-owned marker matching the target with no active journal is trusted for the fast path; every other state takes one snapshot before open and commits only after integrity validation. This recovery path has separate C4A-03 performance evidence | Current V1–V4 are already minor-unit; no destructive rewrite justified; `COM_C4A_EXECUTION_PACKET.md` | P0 / COM-C4 and 1.0 |
