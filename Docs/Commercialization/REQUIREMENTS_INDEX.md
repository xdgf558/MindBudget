...
|---|---|---|---|---|---|---|
| REQ-R1-NET-001 | Active; C3-03A complete and merged, C3-03B In Progress with no transport yet | v1.4 §1 rules 21–23, §4.7, §15.1 | COM-C0B policy; COM-C3 config; COM-C5 telemetry; COM-C6 verification | DEC-COM-021 accepts exact dev/staging/prod hosts, anonymous `GET /v1/config`, app/config-version-only outbound metadata, Ed25519 exact-byte verification, exact whole-second UTC timestamps, duplicate-key rejection, seven-day maximum validity, serialized rollback/equivocation protection, bounded signed cache, sticky corrupt-state failure, abstraction-level write readback, and conservative fallback. It deliberately rejects client-side canonical JSON and defers non-content reason-code logging to C3-03B. C3-03A passed independent review and green GitHub Actions run `31856271268`, then merged through PR #36 as `1ebb36c`; it retains an empty Release HTTP(S) allow-list. C3-03B must prove the fixed adapter, key/Worker provenance, privacy/log/TTL/redirect controls, captured traffic, and final binary before any egress can be accepted. No ledger/content request in R1 | SPEC-001/012 accepted; DEC-COM-021 | P0 / COM-C3, COM-C6 and 1.0 |
...
| REQ-STOREKIT-LIFECYCLE-001 | Active; COM-C2, C3-01, and C3-02 complete and merged; C3-03 In Progress without changing StoreKit authority | v1.4 §§7.1–7.8, 8.3 | COM-C2/C3; server verification C7/C10 | One `EntitlementStore` authority/lifecycle task; typed purchase/restore; verified status/renewal facts; publish before required `finish`; failed finish retry; pending/cancel/error grant nothing. C2-04 completed verified `AppTransaction` bundle/environment agreement and exact environment isolation. PR #33 merged C3-01 voluntary Settings/value-trigger presentation, StoreKit `displayPrice`, optional actual offer plus fresh eligibility, explicit purchase/restore/manage controls, and HKG/USA/SGP/TWN test probes as `747b628` after green CI. Exact P1W is fixture evidence only; production catalog/authority ignores promotion presence and duration. Eligible paid/unknown introductory modes retain their StoreKit price/mode but pause purchase in both View and adapter; promotion shape never gates existing rights. Unavailable subscription authority blocks purchase in both View and actor with explicit recheck, and renewal disclosure uses the app locale. C3-02 projects active trial only from a verified current free-trial transaction plus verified renewal information, separates the current trial product from the accepted `autoRenewPreference` used for next-renewal pricing, schedules one generic local reminder at calendar T−5 only with a reliable date and auto-renew enabled, removes/replaces it on lifecycle changes, and uses an in-app fallback without implicit permission prompting. Pending notification copy never promises that renewal remains enabled after the app stops. PR #34 passed independent review and green GitHub Actions run `31803898776`, then merged C3-02 as `12d9217`. C3-03A configuration is structurally incapable of changing these facts, StoreKit price/trial, or entitlement. Formal products/final economics, C3-03B/C3-04, and distribution remain later gates | SPEC-005/006/014/017 accepted; DEC-COM-015/016/017/018/019/020/021 | P0 / paid TestFlight and 1.0 |
| REQ-MONEY-001 | Active; baseline satisfied, delta pending | v1.4 §1 rule 25, §9.2, §9.6.4 | Existing product; COM-C4A delta; shared with Watch | Every stored/calculated amount is `Int64` minor units plus ISO code; currency exponent matrix; overflow/cross-currency failure; money-path float gate | SPEC-015 accepted tooling boundary | P0 / all financial phases |
