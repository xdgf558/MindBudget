...
- Corrected Insights so the point-in-time "budget still has a buffer" result is no longer stored
  ...
  category breakdown is now a localized donut chart. It shows the five largest categories and a
  combined remainder so every recorded amount in the 30-day total remains represented.
